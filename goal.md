...
> 恢复后的执行代理：使用 `superpowers:executing-plans` 按阶段执行，以复选框跟踪进度。用户明确恢复前，以本文件的暂停状态为准，不自动续跑。原执行授权涵盖按计划完成设计、代码修改、本地运行、模拟器调试与验证；恢复后常规设计取舍和可逆操作自主推进，不逐页请求批准。
...
状态：已暂停（2026-09-14，按用户要求）。本轮仅整理既有工作区改动、执行提交前检查、保存阶段提交并核对完成进度；停止后续功能实施、设备验收和实时读取，待用户明确恢复后继续。暂停不代表目标完成。

证据管理更新（2026-09-14）：按用户新要求，运行截图和录屏保存在仓库外，Git仅保留文字、元数据、摘要及必要原型源文件。此前40个未发布提交中的验收截图已移除，代码与测试内容不变；下文历史SHA可通过[提交映射](docs/migration/evidence/2026-09-14-ui-ux-commit-map.txt)定位。归档规则见[证据说明](docs/design/evidence/README.md)，目标继续暂停。

暂停整理结果：P5-F已有Android旧版拍照接线、共享表单/宿主/平台测试与20张合成原生图已保存于 `b5fa7dfebf360b5029033bfc628caaa39e68e5cc`。当前候选723项Flutter、`just check`（含CLI端到端16项）、冻结引用、结构和敏感检查及Android Debug APK构建均通过。真实相机、手机/平板系统选图、Windows/Linux照片、P4余项、P5全领域与P6/P7仍未完成；本轮不开展设备或实时读取验收。完整进度及恢复顺序见[暂停交接记录](docs/migration/evidence/2026-09-14-ui-ux-pause.md)。

历史P5-E已取得四端系统选图接线、三端原生探测/表单r3与717项门禁；macOS浅色系统选择、取消保原图、重选、超限拒绝、清除5项实操通过。生产Direct/WebVPN表单只读已分批通过。此前锁屏与输入限制保留为当时事实，恢复时重新检查环境；历史通过记录不替代最终候选验收，84个稳定编号保持不变。
...
| `docs/design/evidence/ui-ux/` | 版本化原型源文件、文字记录、设备/状态元数据、外部归档索引与摘要；截图/录屏本体保存到仓库外的 `UBAA_UI_EVIDENCE_DIR`，逻辑文件名关联页面编号、状态、设备与阶段 |
...
- [ ] 批次 D：博雅课程/详情/已选/统计；图书馆楼馆/分区/座位/时段/记录；研讨室站点/日期/时段/订单/详情；阳光概览/记录。
...
P5-D局部：阳光独立表单、时间/照片/文字草稿及七类内存完整提交两端通过；实际图发现分享重复标题后修复，最终两端复验并归档276图。历史P5-D候选Mac完整42场景已补验通过，生产App两路线表单浏览已于P5-E补验；独立Mac完整交互仍欠验，以下完整领域及生命周期矩阵不据局部场景勾选。

P5-B局部进展：独立表单/类型选择/内存草稿已实现并复验。P5-B2时段常驻筛选已收右上、长表头固定；702项、三端r2各20业务及132两端原图通过，生产双路线日期/站点切换只读通过。Mac独立3图与数字草稿往返已补，换日期时锁屏；P5-C图书馆密度已按旧顺序实现，704项、两端144原图和生产双路线通过，Mac分批原生复验覆盖；独立Mac及所有领域完整写入继续。

P5-A局部进展：统一确认取消/返回/期限保护已实现，三端合成签到路径通过；Mac独立视觉待补，以下全部领域清单仍未完成。

- [ ] 按清单逐项迁移博雅选/退/签到/签退、图书馆预约/取消、研讨室预约/取消、课堂签到、阳光照片表单、评教单门/批量流程；不能只验证一个共享确认弹窗。
...
- 当前为用户要求的暂停状态，以下续跑规则仅在用户明确恢复目标后适用。
