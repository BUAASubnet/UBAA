...
> 当前活动阶段（2026-09-05）：代码与目录结构治理及阶段 14A 的跨平台门禁修复已实现，正在固定新候选验收。
> 旧候选 `d43c177` 的 19 项本地门禁和五平台原生 CI 通过，但合同 CI 暴露工具链输出 Broken pipe 与
> Windows 不稳定文件身份 API 编译问题，因此不能作为最终通过证据。修复与记录随本页所属提交形成新候选，
