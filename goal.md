...
- [x] O3-C实现与手机/平板复验：恢复博雅/图书馆/研讨室子菜单及当前页标题；545项Flutter门禁与两端本批操作通过。macOS原生断言通过，但独立实窗巡检被锁屏阻塞，必须后补。
- [x] O3-D1本批：图书馆公开结构和旧版单页楼馆/楼层/分区/座位选择，558项门禁及手机/平板各46图通过；macOS原生断言、Core-live图书馆双路线通过。静态地图和macOS独立实窗仍须后补，不能据此关闭整个D1。
- [x] O3-D1b局部补齐：29分区静态地图及原始时段三字段回填；561项门禁、手机/平板各58图与macOS18项原生断言通过。独立实窗及最终生产UI仍未完成。
- [x] O3-D2局部：研讨室typed全时段、旧版自然选择/表格/订单；572项门禁、两端各44图、macOS14项原生+6图实窗和Core-live双路线通过。锁屏已解除，前批欠验和最终生产UI继续补。
