...
上游正文；`upstream` 解析/URL 模块为 crate-private。实际 envelope 的 JSON Schema 校验、schema-v5 聚合
...
CLI envelope 的 schema v5 与磁盘 Session 版本相互独立：前者描述 stdout 的封闭公开合同，后者仍是
`session.json` schema v2 双路线快照；本次 CLI 升级不迁移或改写会话文件。Flutter bridge contract v4
同步承载 LibBook 可空整数状态、typed 预约资格与稳定目标；它同样不读取或改写磁盘 Session 合同。
