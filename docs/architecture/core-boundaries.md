...
- `session`: the persistence port, Cookie filtering, locked and atomically replaced `session.json`, and invalidation policy.
- `upstream`: frozen SSO/User Center URLs, HTML form parser, captcha detector, and JSON wrapper parser.
- `runtime` (private): mode, boxed transport/store ports, Cookie jar, timestamps, URL conversion, request execution, persistence, and cleanup.
...
- `facade`: the concrete `UbaaClient`, containing only construction and stable host-facing delegation.
