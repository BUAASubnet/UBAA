...
| Core-live 入口 | `apps/ubaa-cli/src/bin/core-live.rs`、`apps/ubaa-cli/tests/core_live_runtime.rs` | 单个固定路线客户端的一次准备/登录、逐操作只读调用、依赖状态、诊断字段和安全摘要；运行时测试覆盖 auto/凭据失败、敏感输入和会话材料清理，源码不包含任何写操作调用 |
