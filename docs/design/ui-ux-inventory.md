...
| CAP-01 | 照片选择/预览/重新选择 | `U/write/ygdk_form.dart`；`packages/ubaa_host/lib/src/`；`packages/ubaa_platform/lib/` | 平台照片能力，合成图片用于测试；照片不入诊断/证据 | 抽象与内存实现有，原生通道未接线 | 脱敏注入可达，生产不可达 | 打卡表单原位反馈，权限不可用说明 | `UT/widgets/ygdk_writes.dart`；`packages/ubaa_host/test/ubaa_app_host/capability_gates.dart` | P5-D完整709及21阳光聚焦、宿主注销迟到照片1项通过 | P5-D两端及Mac42内存场景通过；生产Direct-r3新增入口缺失，确认为原生photo通道未注册，系统选择器须P5-E实现后再验 | 不适用（本地） |
