...
O3-B更新（待本批复验）：EV-01/EV-02在各端均为旧版紧凑课程行；W-EV-01沿“勾选一门→准备评教→确认”进入原单target流程，W-EV-02沿“勾选多门→准备批量评教→确认”进入原有序targets流程。课程详情图标只显示公开字段，单门/批量业务方法与资格不变；不在每张卡重复常驻按钮。原84编号保留。
