...
审计日期：2026-09-08。静态源码基点：`e6e0e3c5f9c52f9c302742a780da3e67a8251a65`。初始盘点为P1静态审查；现按每行和末尾的分批实现、原生操作及真实只读证据持续更新。未执行条目保持未执行，历史PASS不自动继承到新候选。
...
每行编号固定，设计和运行证据引用编号，不随导航改名。实现列指代码存在程度，UI 可达列只指静态接线；每行后三项分别为确定性验证、实际渲染、真实只读，各阶段证据独立记账。真实学校写入禁止；写入列标“不适用（写入）”，准备/提交/回读仅以显式脱敏backend验证，不冒称真实业务验收。
...
- 当前页面地图：`U/app/shell.dart` 的主页、普通功能、高级功能；资料、设置、关于从侧栏进入；普通功能包含课表/考试/成绩/博雅/空教室/SPOC/希冀/图书馆，高级功能包含课堂签到/研讨室预约/阳光/评教。来源 `packages/ubaa_domain/lib/src/feature/catalog.dart`。查询收于顶栏右上面板，关闭保留草稿；博雅/图书馆/研讨室沿旧子菜单与领域详情，低频字段按需展开，具体入口及欠验见各行。
- 主页先展示今日课表与待办，可沿条目直达对应详情；从普通/高级分组卡片进入领域，查询视图与参数仅按需操作。完整任务点击数/重复输入基线仍待P1/P7逐任务闭合，不能把旧默认首页卡片点击数冒称现行实测。

## 认证、个人与全局

| 编号 | 用户任务与当前入口 | 实现位置与约束 | 实现 | UI 可达 | 新设计位置/接入结论 | 对应测试 | 确定性验证 | 实际渲染 | 真实只读 |
|---|---|---|---|---|---|---|---|---|---|
| AUTH-01 | 启动与会话恢复：启动页 | F/auth.rs auth_status；B/client.rs auth_status；A/controller/app_controller.dart；U/app/splash.dart；保留部分路线就绪 | 有 | 自动 | 启动页保留恢复/失败分支 | AT/app_controller/auth.dart；AT/app_controller/lifecycle.dart | E4登录三项RED→GREEN；688项Flutter门禁通过，原资料/生命周期/安全存储保护保留 | E4手机r1/平板r2/Mac r4各21业务+1合同；94两端图与Mac5通过/1键盘限制图全核对；独立清除遇锁屏未完成 | E4 Core两路线认证/资料通过；生产App双路线恢复→资料显示/遮罩→设置通过，联系人2，写入0；真实UI密码输入未执行 |
| AUTH-02 | 双路线准备与账号密码登录：登录页 | F/auth.rs prepare_login/login；B/client.rs；A/bridge/common.dart；U/app/login.dart；密码不入日志，成功清除旧意图 | 有 | 有 | 登录页保留错误与部分成功信息 | AT/bridge_backend_characterization/auth.dart；UT/widgets/shell.dart | E4登录三项RED→GREEN；688项Flutter门禁通过，原资料/生命周期/安全存储保护保留 | E4手机r1/平板r2/Mac r4各21业务+1合同；94两端图与Mac5通过/1键盘限制图全核对；独立清除遇锁屏未完成 | E4 Core两路线认证/资料通过；生产App双路线恢复→资料显示/遮罩→设置通过，联系人2，写入0；真实UI密码输入未执行 |
...
| AUTH-04 | 记住密码：登录选项 | U/app/login.dart；packages/ubaa_host/lib/src/；CredentialVault 能力约束 | 有 | 条件可达 | 登录选项解释本机保存及能力不可用 | packages/ubaa_platform/test/credentials_test.dart；packages/ubaa_host/test/ubaa_app_host/capability_gates.dart | E4登录三项RED→GREEN；688项Flutter门禁通过，原资料/生命周期/安全存储保护保留 | E4手机r1/平板r2/Mac r4各21业务+1合同；94两端图与Mac5通过/1键盘限制图全核对；独立清除遇锁屏未完成 | 不适用（本地） |
| AUTH-05 | 自动登录：登录选项 | A/controller/app_controller.dart；U/app/login.dart；受保存凭据和平台能力约束 | 有 | 条件可达 | 随记住密码控制，不绕过恢复失败 | AT/app_controller/auth.dart；packages/ubaa_host/test/lifecycle_test.dart | E4登录三项RED→GREEN；688项Flutter门禁通过，原资料/生命周期/安全存储保护保留 | E4手机r1/平板r2/Mac r4各21业务+1合同；94两端图与Mac5通过/1键盘限制图全核对；独立清除遇锁屏未完成 | 未执行 |
| USR-01 | 查看姓名账号：我的 | F/read/services.rs get_user_info；B/client.rs user_info；A/bridge/common.dart _userInfo 只投影 username/name；U/app/profile.dart | 有 | 有 | 侧栏资料摘要 | AT/bridge_backend_characterization/auth.dart；UT/widgets/shell.dart | E4登录三项RED→GREEN；688项Flutter门禁通过，原资料/生命周期/安全存储保护保留 | E4手机r1/平板r2/Mac r4各21业务+1合同；94两端图与Mac5通过/1键盘限制图全核对；独立清除遇锁屏未完成 | E4 Core两路线认证/资料通过；生产App双路线恢复→资料显示/遮罩→设置通过，联系人2，写入0；真实UI密码输入未执行 |
| USR-02 | 查看其余公开资料 | B/client.rs user_info 白名单含 schoolId/email/phone/idCardTypeName；A 的 UserSummary 已同次投影四字段 | 有（A新实现） | 我的→查看账号资料 | 本地展开，联系人默认遮罩；主动显示，不接证件号码 | AT/profile_projection_test.dart；Domain/user_summary_test.dart；UT/profile_details_test.dart | E4登录三项RED→GREEN；688项Flutter门禁通过，原资料/生命周期/安全存储保护保留 | E4手机r1/平板r2/Mac r4各21业务+1合同；94两端图与Mac5通过/1键盘限制图全核对；独立清除遇锁屏未完成 | E4 Core两路线认证/资料通过；生产App双路线恢复→资料显示/遮罩→设置通过，联系人2，写入0；真实UI密码输入未执行 |
| SET-01 | 默认路线设置：我的→连接模式 | F/routing.rs；B/client.rs set_default_route_policy；A/bridge/common.dart；U/app/profile.dart；请求中拒绝、保存失败保留旧策略、重开后旧 intent 失效 | 有 | 有 | 侧栏设置→连接设置 | AT/app_controller/lifecycle.dart；CT/route_policy.rs | E4登录三项RED→GREEN；688项Flutter门禁通过，原资料/生命周期/安全存储保护保留 | E4手机r1/平板r2/Mac r4各21业务+1合同；94两端图与Mac5通过/1键盘限制图全核对；独立清除遇锁屏未完成 | 不适用（本地设置） |
| SET-02 | 查看已认证路线：我的 | B/client.rs route_settings；U/app/profile.dart activeRoutes；不等于本次请求路线 | 有 | 有 | 连接设置与状态摘要 | UT/widgets/shell.dart；AT/app_controller/auth.dart | E4登录三项RED→GREEN；688项Flutter门禁通过，原资料/生命周期/安全存储保护保留 | E4手机r1/平板r2/Mac r4各21业务+1合同；94两端图与Mac5通过/1键盘限制图全核对；独立清除遇锁屏未完成 | E4 Core两路线认证/资料通过；生产App双路线恢复→资料显示/遮罩→设置通过，联系人2，写入0；真实UI密码输入未执行 |
...
| SET-04 | 明暗主题与选择 | Host lifecycle.dart内存_themeMode；callbacks.dart传值；U/app/profile.dart选择 | 有 | 我的→外观主题 | 系统/浅色/深色，本次运行生效，重启跟随系统 | Host theme_preference_test.dart；UT/widgets/goldens.dart | E4登录三项RED→GREEN；688项Flutter门禁通过，原资料/生命周期/安全存储保护保留 | E4手机r1/平板r2/Mac r4各21业务+1合同；94两端图与Mac5通过/1键盘限制图全核对；独立清除遇锁屏未完成 | 不适用（本地） |
| SET-05 | 匿名统计开关：我的 | U/app/profile.dart；packages/ubaa_platform/lib/；只计功能使用，不包含内容 | 有 | 有 | 侧栏设置→隐私；保持默认和声明一致 | packages/ubaa_platform/test/telemetry_test.dart | E4登录三项RED→GREEN；688项Flutter门禁通过，原资料/生命周期/安全存储保护保留 | E4手机r1/平板r2/Mac r4各21业务+1合同；94两端图与Mac5通过/1键盘限制图全核对；独立清除遇锁屏未完成 | 不适用（本地） |
| SET-06 | 查看/复制本次诊断：登录或我的 | U/app/profile.dart；U/common/error_card.dart；A/controller/app_controller/diagnostics.dart；不自动上传 | 有 | 有 | 错误页与侧栏设置→诊断；仅脱敏元信息 | UT/diagnostics_test.dart；AT/diagnostics_test.dart | E4登录三项RED→GREEN；688项Flutter门禁通过，原资料/生命周期/安全存储保护保留 | E4手机r1/平板r2/Mac r4各21业务+1合同；94两端图与Mac5通过/1键盘限制图全核对；独立清除遇锁屏未完成 | 不适用（本地） |
| AUTH-06 | 退出：我的 | F/auth.rs logout；B/client.rs logout；U/app/profile.dart；保留主动保存凭据，清理会话/意图 | 有 | 有 | 侧栏资料/设置底部退出 | AT/app_controller/lifecycle.dart；packages/ubaa_host/test/lifecycle_test.dart | E4登录三项RED→GREEN；688项Flutter门禁通过，原资料/生命周期/安全存储保护保留 | E4手机r1/平板r2/Mac r4各21业务+1合同；94两端图与Mac5通过/1键盘限制图全核对；独立清除遇锁屏未完成 | 未执行 |
| AUTH-07 | 退出并清除本机账号：我的→确认 | U/app/profile.dart _confirmClearAccount；宿主 CredentialVault 清理；不删除学校数据 | 有 | 有 | 独立破坏性本机确认 | packages/ubaa_host/test/ubaa_app_host/callbacks.dart；packages/ubaa_platform/test/credentials_test.dart | E4登录三项RED→GREEN；688项Flutter门禁通过，原资料/生命周期/安全存储保护保留 | E4手机r1/平板r2/Mac r4各21业务+1合同；94两端图与Mac5通过/1键盘限制图全核对；独立清除遇锁屏未完成 | 不适用（本地） |
| NAV-01 | 导航、返回与刷新 | U/app/{shell,home}.dart；A/controller/app_controller/refresh.dart；generation/迟到响应保护 | 有 | 有 | 旧三导航与单顶栏；首页今日课表→六来源待办，点击直达原详情，返回保留滚动 | D5 Domain/home_todo、controller/home_sources、reminders、宿主home_navigation ；侧栏关于单卡、真实版本与两个公开外链 | D5 617项Flutter门禁；时间边界/缓存并发/提醒隔离/短路线面板通过 ；E5 navigation/about与platform/app_information | D5手机平板各54图全复核，macOS16原生及6独立实窗；最终P6仍待验 ；E5完整715项通过 | D5生产App两路线8来源均通过，已选0条；真实写入0 ；E5两端各11原生场景/29图、Safari双链接往返通过，Mac关于未通过 |
...
|---|---|---|---|---|---|---|---|---|---|---|
| SCH-01 | 查看今日课表 / 今日课程 | `F/read/academic.rs schedule_today`；`academic.dart` | 无参数；课程时间地点可空 | 有 | 有 | 右上查询保留今日课程；默认入口采用唯一当前学期/周 | App academic_weeks；Domain schedule_dates；Flutter schedule/navigation；原生 ui_schedule_navigation | E3c全Flutter685通过；缓存隔离/歧义/草稿/日期/重试/长名RED→GREEN | E3c三端r3各24业务；手机/平板各62及Mac原生检查点5图全核对；独立鼠标及完整物理键盘仍待P6 | E3c生产双路线当周10→邻周11→另一学期1→今日1，前三周各7真实日期，草稿/actualroute通过，写入0；原列表链见E3b |
| SCH-02 | 选择学期 / 学期列表 | `F/read/academic.rs schedule_terms`；`academic.dart` | 真实 itemCode 与 selected；不猜学期 | 有 | 有 | 右上按需学期选择；换学期清旧周次，原学期列表保留 | App academic_weeks；Domain schedule_dates；Flutter schedule/navigation；原生 ui_schedule_navigation | E3c全Flutter685通过；缓存隔离/歧义/草稿/日期/重试/长名RED→GREEN | E3c三端r3各24业务；手机/平板各62及Mac原生检查点5图全核对；独立鼠标及完整物理键盘仍待P6 | E3c生产双路线当周10→邻周11→另一学期1→今日1，前三周各7真实日期，草稿/actualroute通过，写入0；原列表链见E3b |
| SCH-03 | 选择教学周 / 周次列表 | `F/read/academic.rs schedule_weeks`；`academic.dart` | 必填 term，日期/当前周来自返回 | 有 | 有 | 右上原教学周选项/前后周；关闭保草稿，明确应用后读取；原周次列表保留 | App academic_weeks；Domain schedule_dates；Flutter schedule/navigation；原生 ui_schedule_navigation | E3c全Flutter685通过；缓存隔离/歧义/草稿/日期/重试/长名RED→GREEN | E3c三端r3各24业务；手机/平板各62及Mac原生检查点5图全核对；独立鼠标及完整物理键盘仍待P6 | E3c生产双路线当周10→邻周11→另一学期1→今日1，前三周各7真实日期，草稿/actualroute通过，写入0；原列表链见E3b |
| SCH-04 | 查看周课表 / 周课表 | `F/read/academic.rs schedule_week`；`academic.dart` | term、正整数week；B1已保留起止时间/节次/周几typed字段，未知值不丢弃 | 有 | 有 | 旧七日节次网格/原色/本地详情；真实起止日期及原周名称，失败不伪造日期 | App academic_weeks；Domain schedule_dates；Flutter schedule/navigation；原生 ui_schedule_navigation | E3c全Flutter685通过；缓存隔离/歧义/草稿/日期/重试/长名RED→GREEN | E3c三端r3各24业务；手机/平板各62及Mac原生检查点5图全核对；独立鼠标及完整物理键盘仍待P6 | E3c生产双路线当周10→邻周11→另一学期1→今日1，前三周各7真实日期，草稿/actualroute通过，写入0；原列表链见E3b |
...
| SIG-01 | 今日课堂列表 / 全部课程 | `F/read/assignments.rs signin_today`；`assignments.dart` | 当天安排、状态可空；独立 iClass 会话在 Core | 有 | 有 | 旧横卡与本地详情；顶栏右上全部/可签到/已签到显式视图，关闭保未应用草稿，返回保已应用查询 | integration_test/ui_academic_old/signin_queries_test.dart与live_readonly_test.dart；原App资格投影不变 | E6原生行为RED→GREEN；App54与analyze、just check通过；E5全715为未改生产实现的最近完整门禁 | E6手机/平板各2主题完整流程与12原图；Mac2主题/27秒通过；独立Mac待补，既有E1其他状态单列 | E6 Core双路线3认证+今日2条；生产Direct65秒/WebVPN107秒，全部2→可签到1→已签到1→全部2、本地详情无额外读取、actualroute一致，学校写入0 |
| SIG-02 | 可签到课堂 / 可签到 | `F/read/assignments.rs signin_today`；`assignments.dart` | 本地仅 allowed 资格筛选；未知不等于未签到 | 有 | 有 | 旧横卡与本地详情；顶栏右上全部/可签到/已签到显式视图，关闭保未应用草稿，返回保已应用查询 | integration_test/ui_academic_old/signin_queries_test.dart与live_readonly_test.dart；原App资格投影不变 | E6原生行为RED→GREEN；App54与analyze、just check通过；E5全715为未改生产实现的最近完整门禁 | E6手机/平板各2主题完整流程与12原图；Mac2主题/27秒通过；独立Mac待补，既有E1其他状态单列 | E6 Core双路线3认证+今日2条；生产Direct65秒/WebVPN107秒，全部2→可签到1→已签到1→全部2、本地详情无额外读取、actualroute一致，学校写入0 |
| SIG-03 | 已签到课堂 / 已签到 | `F/read/assignments.rs signin_today`；`assignments.dart` | 本地只筛Core原denied；当前Core由非空目标与signStatus=1派生；UI不改变资格 | 有 | 有 | 旧横卡与本地详情；顶栏右上全部/可签到/已签到显式视图，关闭保未应用草稿，返回保已应用查询 | integration_test/ui_academic_old/signin_queries_test.dart与live_readonly_test.dart；原App资格投影不变 | E6原生行为RED→GREEN；App54与analyze、just check通过；E5全715为未改生产实现的最近完整门禁 | E6手机/平板各2主题完整流程与12原图；Mac2主题/27秒通过；独立Mac待补，既有E1其他状态单列 | E6 Core双路线3认证+今日2条；生产Direct65秒/WebVPN107秒，全部2→可签到1→已签到1→全部2、本地详情无额外读取、actualroute一致，学校写入0 |
...
| LIB-01 | 楼馆列表 / 馆列表 | `F/read/services.rs libbook_libraries`；`libbook.dart` | day与typed嵌套storeys，保留同名楼层的独立ID | 有 | 有 | 右上按需楼馆→楼层→分区及原日期/时段；全部原参数在更多查询，关闭保草稿/座位 | `UT/widgets/libbook_queries.dart`；`AT/bridge_backend_characterization/read.dart` | P5-C704项门禁；12 App+3 UI聚焦，原日期必填/三字段回填/选择隔离与地图通过 | P5-C手机/平板完整r2各22业务+2合同；144原图核对。Mac分批覆盖（r1其余通过+map-r2补2业务）；独立实窗另记，最终P6待验 | P5-C Core双路线3认证+5读PASS；AppDirect41秒/WebVPN43秒按需面板→分区时段→175座位→2记录只读通过，写入0；不同日期/所有分区未扩大验收 |
| LIB-02 | 馆区和楼层 / 馆区列表 | `F/read/services.rs libbook_areas`；`libbook.dart` | typed父馆/楼层/day；完整手填仍在顶栏面板 | 有 | 有 | 右上按需楼馆→楼层→分区及原日期/时段；全部原参数在更多查询，关闭保草稿/座位 | `UT/widgets/libbook_queries.dart`；`AT/bridge_backend_characterization/read.dart` | P5-C704项门禁；12 App+3 UI聚焦，原日期必填/三字段回填/选择隔离与地图通过 | P5-C手机/平板完整r2各22业务+2合同；144原图核对。Mac分批覆盖（r1其余通过+map-r2补2业务）；独立实窗另记，最终P6待验 | P5-C Core双路线3认证+5读PASS；AppDirect41秒/WebVPN43秒按需面板→分区时段→175座位→2记录只读通过，写入0；不同日期/所有分区未扩大验收 |
| LIB-03 | 分区详情 / 分区详情 | `F/read/services.rs libbook_area_detail`；`libbook.dart` | areaId；营业窗口由 upstream 条件决定 | 有 | 有 | 右上按需楼馆→楼层→分区及原日期/时段；全部原参数在更多查询，关闭保草稿/座位 | `UT/widgets/libbook_queries.dart`；`AT/bridge_backend_characterization/read.dart` | P5-C704项门禁；12 App+3 UI聚焦，原日期必填/三字段回填/选择隔离与地图通过 | P5-C手机/平板完整r2各22业务+2合同；144原图核对。Mac分批覆盖（r1其余通过+map-r2补2业务）；独立实窗另记，最终P6待验 | P5-C Core双路线3认证+5读PASS；AppDirect41秒/WebVPN43秒按需面板→分区时段→175座位→2记录只读通过，写入0；不同日期/所有分区未扩大验收 |
| LIB-04 | 可用日期/时段选择 | `F/read/services.rs libbook_area_detail`；`libbook.dart` | 保留availableDates与时段id/start/end/label；没有日期关联 | 有，受日期关联上限限制 | 时段三字段回填，明确日期后完整查询可达 | 右上按需楼馆→楼层→分区及原日期/时段；全部原参数在更多查询，关闭保草稿/座位 | `UT/widgets/libbook_queries.dart`；`AT/bridge_backend_characterization/read.dart` | P5-C704项门禁；12 App+3 UI聚焦，原日期必填/三字段回填/选择隔离与地图通过 | P5-C手机/平板完整r2各22业务+2合同；144原图核对。Mac分批覆盖（r1其余通过+map-r2补2业务）；独立实窗另记，最终P6待验 | P5-C Core双路线3认证+5读PASS；AppDirect41秒/WebVPN43秒按需面板→分区时段→175座位→2记录只读通过，写入0；不同日期/所有分区未扩大验收 |
| LIB-05 | 座位查询 / 座位查询 | `F/read/services.rs libbook_seats`；`libbook.dart` | areaId/day/start/end；app 另必填 segment 供预约，不能猜编号 | 有 | 有 | 四列座位，选中后才显示摘要；仅独立allowed完整action可准备 | `UT/widgets/libbook_queries.dart`；`AT/bridge_backend_characterization/read.dart` | P5-C704项门禁；12 App+3 UI聚焦，原日期必填/三字段回填/选择隔离与地图通过 | P5-C手机/平板完整r2各22业务+2合同；144原图核对。Mac分批覆盖（r1其余通过+map-r2补2业务）；独立实窗另记，最终P6待验 | P5-C Core双路线3认证+5读PASS；AppDirect41秒/WebVPN43秒按需面板→分区时段→175座位→2记录只读通过，写入0；不同日期/所有分区未扩大验收 |
| LIB-06 | 我的预约 / 预约记录 | `F/read/services.rs libbook_bookings`；`libbook.dart` | page>=1、limit 1–100；保留服务端分页 | 有 | 有 | 旧版紧凑记录；编号进只读详情，canonical取消保留原页 | `UT/widgets/libbook_writes.dart`；`AT/bridge_backend_characterization/read.dart` | P5-C704项门禁；12 App+3 UI聚焦，原日期必填/三字段回填/选择隔离与地图通过 | P5-C手机/平板完整r2各22业务+2合同；144原图核对。Mac分批覆盖（r1其余通过+map-r2补2业务）；独立实窗另记，最终P6待验 | P5-C Core双路线3认证+5读PASS；AppDirect41秒/WebVPN43秒按需面板→分区时段→175座位→2记录只读通过，写入0；不同日期/所有分区未扩大验收 |
| CG-01 | 研讨室站点 / 站点列表 | `F/read/services.rs cgyy_sites`；`cgyy.dart` | 站点 ID 来自返回 | 有 | 有 | 右上校区→日期→楼栋/楼层按需选择；正文保实际地点日期/时段表，完整参数仍可达 | `UT/widgets/queries.dart`；`AT/bridge_backend_characterization/read.dart` | P5-B2完整702项；面板关闭保选择且不额外读、42行固定表头/双向横滚通过 | P5-B2三端r2各20业务；手机/平板132原图全数核对；Mac独立3图及数字草稿往返通过，独立换日期时锁屏；最终P6待验 | P5-B2 Core双路线3认证+6读PASS；生产AppDirect37秒/WebVPN32秒，日期/站点切换保日期、关闭不额外读及actualRoute通过，写入0 |
| CG-02 | 用途 / 用途类型 | `F/read/services.rs cgyy_purpose_types`；`cgyy.dart` | 来源 upstream/static_fallback 必须明示 | 有 | 有 | 独立用途查询与来源已保留；表单typed用途选择随P5补齐 | `UT/widgets/queries.dart`；`AT/bridge_backend_characterization/read.dart` | P5-B700项门禁；用途独立缓存4项、表单回归6项及原typed写入/取消保护通过 | P5-B三端r3各20业务；两端132原图已核对；预约表单/prepare/cancel及订单取消prepare/cancel通过，全部commit异常原生矩阵仍待；Mac独立实窗锁屏 | P5-B Core双路线用途10项static_fallback；生产App双路线独立用途不覆盖时段、表单/类型列表/返回通过，写入0 |
| CG-03 | 日期空间 / 日期空间 | `F/read/services.rs cgyy_day_info`；`cgyy.dart` | 正 siteId、严格日期 | 有 | 有 | 右上校区→日期→楼栋/楼层按需选择；正文保实际地点日期/时段表，完整参数仍可达 | `UT/widgets/queries.dart`；`AT/bridge_backend_characterization/read.dart` | P5-B2完整702项；面板关闭保选择且不额外读、42行固定表头/双向横滚通过 | P5-B2三端r2各20业务；手机/平板132原图全数核对；Mac独立3图及数字草稿往返通过，独立换日期时锁屏；最终P6待验 | P5-B2 Core双路线3认证+6读PASS；生产AppDirect37秒/WebVPN32秒，日期/站点切换保日期、关闭不额外读及actualRoute通过，写入0 |
...
| YG-01 | 学期概览 / 概览 | `F/read/services.rs ygdk_overview`；`ygdk.dart` | 分类/项目/学期次数与可空目标；重复 target 不允许写入 | 有 | 有 | 学期/本周概要→账号隔离首页提醒开关→记录；右下新增→独立表单内选择项目 | D4 App投影 / UI ygdk_content / 宿主sports_navigation | D4概要与回读通过；D5提醒4/16旧规则、账号周学期存储和开关往返通过 | P5-D两端42业务完整r1及16受影响业务r2通过，每端138图；Mac当前候选待解锁补验；D4/D5历史另存 | P5-D Core-live Direct/WebVPN各3认证+2阳光读通过；生产App新独立表单浏览待Mac解锁，不沿用D4作为新表单证据 |
| YG-02 | 记录与分页 / 记录列表 | `F/read/services.rs ygdk_records`；`ygdk.dart` | page/size；公开状态/地点/图片数量，不返回图片 URL | 有 | 有 | 旧记录卡→本地低频详情；首页加载更多，独立记录一基分页 | D4 App投影 / UI ygdk_content / 宿主sports_navigation | 同路线追加与局部失败、低频搜索、分页草稿保留通过 | P5-D两端42业务完整r1及16受影响业务r2通过，每端138图；Mac当前候选待解锁补验；D4/D5历史另存 | P5-D Core-live Direct/WebVPN各3认证+2阳光读通过；生产App新独立表单浏览待Mac解锁，不沿用D4作为新表单证据 |
...
| W-CG-01 | 研讨室预约 / 准备研讨室预约→表单 | `F/write/reservations.rs preflight_cgyy_reservation/cgyy_submit_reservation`；`U/features/cgyy.dart` | 1–2 同空间目标，站点日期时间顺序；电话/主题/用途/人数/正文/参与人及标志 | 有 | 条件可达 | U/write/cgyy_form.dart；用途列表联动，展示完整预约时间 | `UT/widgets/cgyy_writes.dart`；`B/write/tests/cgyy_reservation.rs`（预约）或 `B/write/tests/cgyy_cancel.rs`（取消） | P5-B700项门禁；用途独立缓存4项、表单回归6项及原typed写入/取消保护通过 | P5-B三端r3各20业务；两端132原图已核对；预约表单/prepare/cancel及订单取消prepare/cancel通过，全部commit异常原生矩阵仍待；Mac独立实窗锁屏 | 不适用（写入） |
| W-CG-02 | 研讨室取消 / 准备取消订单 | `F/write/reservations.rs preflight_cgyy_cancel/cgyy_cancel_order_if_route_matches`；`U/features/cgyy.dart` | 正订单 ID、typed 取消资格、原路线核对 | 有 | 条件可达 | 订单详情和订单卡一致动作，核对取消状态 | `UT/widgets/cgyy_cancel_writes.dart`；`B/write/tests/cgyy_reservation.rs`（预约）或 `B/write/tests/cgyy_cancel.rs`（取消） | P5-B700项门禁；用途独立缓存4项、表单回归6项及原typed写入/取消保护通过 | P5-B三端r3各20业务；两端132原图已核对；预约表单/prepare/cancel及订单取消prepare/cancel通过，全部commit异常原生矩阵仍待；Mac独立实窗锁屏 | 不适用（写入） |
| W-YG-01 | 阳光照片打卡 / 准备阳光打卡→表单 | `F/write/campus.rs preflight_ygdk_submit/ygdk_submit_if_route_matches`；`U/features/ygdk.dart` | 分类项目 target、开始结束、地点可选、公开开关、照片 bytes/name/MIME；一次 upload/final | 有 | 条件可达 | 独立页面按旧项目→时间→地点→照片→分享；同代次文字草稿，退出释放照片 | UI widgets/ygdk_form.dart与ygdk_writes.dart；宿主注销保护；原生ui_sports/form.dart | 21阳光聚焦及宿主注销通过；709全门禁，分享修复后UI241通过；原输入校验/单次提交/unknown与回读保护保留 | P5-D手机/平板完整42业务及受影响16业务复验通过，七类内存写入两主题；276图见old-p5d-native；Mac当前候选仍待补 | 不适用（写入） |
...
| CAP-01 | 照片选择/预览/重新选择 | `U/write/ygdk_form.dart`；`packages/ubaa_host/lib/src/`；`packages/ubaa_platform/lib/` | 平台照片能力，合成图片用于测试；照片不入诊断/证据 | iOS/macOS/Android/OHOS系统图片选择已接线；Android拍照及Win/Linux待补 | 当前三端表单可达；macOS系统选图5项实操通过，手机/平板待验 | 打卡表单原位反馈，权限不可用说明 | `UT/widgets/ygdk_writes.dart`；`packages/ubaa_host/test/ubaa_app_host/capability_gates.dart` | P5-E原生探测/平台错误RED→GREEN，Swift实际图片边界通过，最终717；更换失败保原图/大小访问说明RED→GREEN | P5-E三端最终r3各3项真实平台探测+内存业务表单通过，两端16原图复核；生产双路线可进入表单，0 prepare；9/12补macOS浅色真实选择/取消保图/重选/超限保图/清除5项与5张原图；手机/平板系统选图仍因锁屏待验，不能视作照片完整通过 | 不适用（本地） |
...
| W-COM-01 | 准备→确认→取消/提交 | `A/write/coordinator.dart`；`B/write/lifecycle.rs`；`U/write/confirmation.dart` | 120 秒 intent；取消失败保留阻塞；只用 opaque ID commit；账户/路线切换失效 | 有 | 全写入共用 | 顶栏固定实际路线与取消返回；正文目标/警示/期限；原到期自动禁用，忙碌中不能取消或双击 | `UT/write_coordination_test.dart`；`AT/write_coordinator/flow.dart`；`AT/write_coordinator/invalidation.dart` | P5-A四项行为RED→GREEN；692项Flutter门禁通过；原协调器/资格/生命周期保护保留 | P5-A三端r4各20合成场景（含取消重试、busy、过期、未知/异常/回读）；手机/平板各46原图已核对；Mac独立实窗锁屏待补；不覆盖其他领域全部写入 | 不适用（写入） |
...
P5-D进行中（2026-09-10）：W-YG-01独立表单/同读取代次文字草稿、日期时间选择、照片取消更换保留和关闭释放已实现；聚焦与注销宿主回归通过。新增显式SportsWriteBackend仅内存，用于成功/未知/回读失败/上传失败/取消失败/过期/提交中七类两主题场景，三端尚在执行，不提前标通过。YG-01原新增前置项目弹窗改为旧版表单内选择，YG-02读取协议及分页保持。84编号不变。
