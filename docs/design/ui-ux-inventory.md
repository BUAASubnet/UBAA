...
| YG-01 | 学期概览 / 概览 | `F/read/services.rs ygdk_overview`；`ygdk.dart` | 分类/项目/学期次数与可空目标；重复 target 不允许写入 | 有 | 有 | 学期/本周概要→记录列表；右下新增才选择项目 | D4 App投影 / UI ygdk_content / 宿主sports_navigation | 零/null/独立统计、显式同路线组合、后台与固定回读不额外Auto通过 | D4手机平板各r1全28场景/r2正常两主题；macOS28原生+4独立实窗，old-d4-native | D4 Core-live及生产App Direct/WebVPN均通过；真实写入0 |
