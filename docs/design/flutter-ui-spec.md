...
取消入口状态门禁：图书馆预约详情只有在状态码不为冻结的结束/取消值、状态名不含已取消/结束/完成/过期/失效语义时
显示取消准备按钮；场馆订单显示冻结状态说明，并同时要求 `orderStatus`/`checkStatus` 为可取消状态，在预约开始前四小时截止前才显示。
缺少或无法解析场馆状态时隐藏入口，由 Core prepare 做最终校验；该入口只展示公开 ID，不显示上游令牌或内部字段。
...
- 十二项功能均已通过共享详情页提供正式导航入口，不再使用生产 Demo backend 或“即将接入”占位页。
  `c2fed18` 已补齐十二项详情的逐领域 golden；博雅/SPOC/Judge/图书馆的领域筛选、服务端分页加载、
  逐领域详情状态和逐领域 integration 的上游/宿主证据，以及写入流程的逐领域结果核对和权限/挑战交互，
  仍需按 P3/P4 继续补齐；本规格因此仍不把 P3/P4 标记为完成。
