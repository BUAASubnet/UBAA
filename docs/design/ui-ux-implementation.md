...
- [ ] 在 `docs/migration/evidence/2026-09-08-ui-ux.md` 记录原型类别、尺寸、截图索引、问题与复验。原型工具或文件由执行代理选择；截图放仓库外的 `UBAA_UI_EVIDENCE_DIR` 并标明 prototype，`docs/design/evidence/ui-ux/` 只保留原型源文件、文字、元数据及摘要，见[证据存储规则](evidence/README.md)。
...
Domain ScheduleCoursePresentation与app映射补公开color；UI schedule_content恢复七日网格/固定节次/重叠课程完整集合，academic/schedule_detail承载本地详情；feature_detail仅明确空周保留网格，未知/失败状态不伪装成功。6项新增行为与原学业回归通过，全Flutter667。ui_schedule_old显式合成backend三端最终复验中；生产只读从今日→原学期列表→原周次列表→周课表，不修改请求协议。E3c自然周选择和真实日期接线仍待做。
...
新增App academic_weeks缓存、Domain周日期标签、UI schedule_flow/week_picker及宿主接线。缓存按学期、生命周期及epoch隔离，合并在途，失败可重试，不覆盖全局快照；自动初始化只在可见且未由用户编辑时推进。先后观察缓存/日期/自然导航/重试/长名称行为RED，再最小修复。最终Flutter685通过，三端r3各24业务通过，129张合成原图全部核对；生产Direct/WebVPN均自然当周→邻周→另一学期→今日通过，真实写入0。Mac额外可选截窗钩子仅合成测试使用，之后单场景及analyze通过；独立CUA鼠标路径与完整物理键盘尚未通过。P4其余旧基准、P5–P7继续。


### E4 登录与账号复核（2026-09-09）

E4生产只改app/login.dart；新增account/login_old三项RED→GREEN，原生ui_account改为侧栏定位，并增加六个登录场景与显式合成inspection。初始Mac未选导航文本不可点击已改原图标定位。688项Flutter门禁通过；之后新增/修正测试辅助经App analyze通过，生产UI未再变化。手机r1/平板r2各21业务+1合同，Macr4同组通过；94两端图及5Mac通过图/1键盘限制图已检查。
