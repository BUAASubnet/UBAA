...
UBAA2 已提供场地、用途、日期、订单、详情和锁码的 Core/CLI 只读入口，并实现独立 token、签名和验证码协议；Cgyy WebVPN 业务会话及部分 Direct 上游失败已记录，写操作仅允许确定性 Mock/向量验证。
...
| Cgyy | 场地、用途、日期可用性、订单、锁码 | 预约、取消、验证码 | 已接入五项只读及锁码 Core/CLI；预约验证码求解、预约、取消具备确定性协议证据；真实写请求禁止；WebVPN 已使用独立 Direct 业务 runtime，日期/订单/锁码实时可用性仍按逐操作结果记录 | 低 |
