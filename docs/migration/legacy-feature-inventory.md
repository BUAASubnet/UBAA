...
当前 UBAA2 CLI 的合同范围是用户信息、课表、考试、成绩、空闲教室、SPOC 作业和希冀作业。旧版还实现了签到（`Signin`）、阳光打卡（`Ygdk`）、图书馆预约（`LibBook`）、场馆预约（`Cgyy`）和博雅课程（`Bykc`）五组 API；其中只有查询部分适合后续只读迁移，写操作必须继续排除在本次 CLI 合同之外。
...
当前 UBAA2 无 `signin` domain、upstream、facade 或 CLI 命令，也没有签到路由策略。查询优先级为中：它是纯 GET，但依赖额外 iClass 登录和日期语义；签到写操作明确不迁移。
...
|---|---|---|---|---|
| Signin | 今日课堂/签到状态 | 执行签到 | 缺失 | 中 |
| Ygdk | 概览、记录分页 | 运动打卡、照片上传 | 缺失 | 中低 |
| LibBook | 图书馆、区域、座位、我的预约 | 预约、取消 | 缺失 | 低 |
