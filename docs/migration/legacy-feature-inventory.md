...
| Cgyy | 场地、用途、日期可用性、订单、详情、锁码 | 预约、取消、验证码 | 已接入六项只读及锁码 Core/CLI；预约验证码求解、预约、取消具备确定性协议证据；真实写请求默认禁止，2026-08-29 已按独立授权完成一次 Direct 预约并取消；Direct 与 WebVPN 均由 facade 绑定各自路线 runtime，日期/订单/详情/锁码实时可用性由 Core-live 逐操作记录 | 低 |
