...
## 2026-09-02 无签名执行目标 P3–P6 验收复核

- 当前提交 `81dd9d2` 在 `apps/ubaa_flutter/integration_test/app_flow_test.dart` 为十项写操作逐项增加
...
- 当前最终提交 `4eaf1dd` 的营业窗口内串行复核 `just verify-live mode=direct` 与 `just verify-live mode=webvpn`，两路线 exit code 均为 0；认证、用户、
...
- Flutter 原生 CI run `33628444289`（提交 `4eaf1dd`）已终态 `success`：Linux、Windows、macOS、iOS simulator、Android APK 五个平台
  Debug job 均通过无签名产物结构检查并上传产物；合同 CI run `33628444204` 同样终态 `success`，`contract-gates`、macOS Rust 和 Windows Rust
...
- 结合十二项详情页面与 golden（`c2fed18`）、共享状态矩阵、完整 typed 查询宿主 smoke、十项写入确认/不确定结果回归、当前双路线只读矩阵、
  ...
  当前最终提交 `4eaf1dd` 已以 `CARGO_INCREMENTAL=0 CARGO_BUILD_JOBS=1 just flutter-codegen-check` 完成并报告 FRB 生成零漂移，生成目录无差异；此前
...
- 提交 `b0c4a77` 进一步收紧签到安全边界：读取状态明确为“已签到”时禁用重复准备按钮并提示不能重复提交，
  缺少状态字段的旧 DTO 仍由 Core prepare 最终校验；新增失败回归后 `ubaa_ui` 全量 37 项通过。该提交的
  合同 CI `33583052957` 与 Flutter 原生五平台 CI `33583052953` 均终态成功，五个平台 Debug job 全部通过
  并上传产物；不包含签名、实体设备或真实写入。
- 提交 `777261f` 增加主页与共享详情页的明亮主题 golden 基线（1280×800）；缺失基线失败后生成并在固定
...
- P0 无签名目标现已闭合：冻结引用、版本锁定、官方五平台 Debug 构建、OHOS API26 无签名 HAP/arm64
...
- 预期失败已观察，修复后 app_controller 聚焦测试 29/29 通过；该修复只收紧 Dart 生命周期，不改变 Core/路线/上游协议，真实 isolate、平台生命周期和内存工具证据仍缺失。

## 2026-09-02 销毁后的在途路线设置不回写策略

- 新增延迟路线切换回归：controller 销毁后，旧实现仍会读取并应用路线设置；修复在 `setRoutePolicy` 的准备、路线读取和错误边界加入 `_disposed` guard。
- 预期失败已观察，修复后 app_controller 聚焦测试 28/28 通过；该修复只收紧 Dart 生命周期，不改变 Core/路线/上游协议，真实 isolate、平台生命周期和内存工具证据仍缺失。
## 2026-09-02 销毁后的在途登录不继续读取或保存

- 新增延迟登录回归：controller 销毁后，旧实现仍会继续读取用户资料；修复在登录、路线状态、用户资料和凭据/UI 边界加入 `_disposed` guard，并让路线状态读取本身不在销毁后回写。
- 预期失败已观察，修复后 app_controller 聚焦测试 27/27 通过；该修复只收紧 Dart 生命周期，不改变 Core/路线/上游协议，真实 isolate、平台登录生命周期和内存工具证据仍缺失。
...
- 统一确认 UI 增加可选 verifier：场馆提交成功且有收据时等待该匹配结果，成功提示“订单列表已核对”，否则保留“请在订单列表核对”；任何核对异常都不重试写请求。官方 Flutter 与 OHOS 宿主均接入该回调。
- 新增 app/widget 回归覆盖匹配与不匹配；本轮只消费已有读取快照，不新增上游协议或真实副作用，真实预约及 P4/P5/P6 其余门禁仍未完成。

## 2026-09-01 场馆订单 bridge 敏感字段边界收紧

- 先在生成 Dart schema 快照中加入失败断言：`BridgeCgyyOrder` 不得出现交易号、手机号、支付状态、活动正文、参与人或审核内部字段；旧生成绑定确实失败，随后再修改 Rust bridge DTO。
- `BridgeCgyyOrder` 现在只跨 FFI 暴露订单编号、站点/日期/空间/校区、时间、状态、主题、用途名称和参与人数等页面白名单字段；`map_cgyy_order` 不再复制交易号、手机号、支付状态、用途原始编号、活动正文、参与人、审核内容、处理原因和备注。
- FRB 绑定由 `flutter_rust_bridge_codegen 2.13.0` 重新生成，聚焦 schema 与 bridge 测试通过；本轮不改变 Cgyy 上游请求、写入次数或订单核对策略，未执行真实写入。

## 2026-09-01 场馆预约收据的 UI 安全呈现

- 场馆预约成功结果若带有 `CgyyReservationReceipt`，统一确认页现在只在成功提示中显示非敏感订单编号，并明确引导用户到订单列表核对；不显示交易号、电话、主题或活动正文。
- widget 回归先把预期文案改为包含合成订单编号并观察旧实现失败，再完成最小呈现修复；无收据的其它写操作文案保持不变。
- 该增量不改变写入次数、Core 协议或刷新策略；真实预约仍未执行，最终核对仍以同路线订单列表/详情为准。
...
## 2026-09-01 DevEco/Command Line Tools API26 复核
...
## 2026-09-01 场馆预约填写确认流程

- 场馆日期空间读取现在逐个投影可预约时段的站点、日期、空间/空间组、时段 ID 与时间白名单；共享 UI 可从该公开摘要打开填写表单，收集联系电话、主题、用途编号、参与人数、活动内容及两个布尔选项。
- 表单校验通过后才调用既有 typed `prepareCgyySubmitReservation` 并进入统一二次确认页；取消、非法输入或准备失败均不触发提交。widget/bridge 回归覆盖公开 ID 选择、字段传递和确认前零提交。
- 验证码材料仍由 Core 受控流程处理，UI 不读取或保存图片/token；本轮未执行真实场馆预约。挑战失败恢复、权限说明、订单匹配核对及其余阳光打卡/评教表单仍待完成。

## 2026-09-01 评教批量确认入口

- 共享评教详情页现在允许用户逐门勾选待评课程，并显示已选数量、全选/取消全选和“准备批量评教”按钮；批量入口只收集读取白名单中的课程/任务/问卷等 typed 字段。
...
## 2026-09-01 阳光打卡表单确认入口

- 共享阳光打卡详情在存在公开“项目编号”时提供表单入口，收集开始/结束时间、地点、分享到广场选项，并要求宿主注入的 picker 返回内存 `YgdkPhotoInput`；独立对话框组件在自身销毁时释放输入控制器。
- 时间和照片校验通过后才调用既有 typed `prepareYgdkSubmit` 并进入统一二次确认；取消、缺照片、平台未接入 picker 或准备失败均不触发提交。widget 回归覆盖内存照片字段、确认前零提交和确认后单次提交。
- 当前 Flutter/OHOS 宿主只接入 prepare 回调，尚未接入相机/相册/桌面文件选择器、权限拒绝路径或真实阳光打卡；照片上传、平台权限和结果核对仍是 P4/P5/P6 未完成项。
...
## 2026-09-02 博雅签到/签退 typed 分支回归

- 博雅正式详情页的写入回归现在同时触发并断言 `signType=1` 签到与 `signType=2` 签退；两次都只创建一次性确认 intent，取消确认不会调用 commit。
- 该补充只验证既有 typed `prepareBykcSignWrite(courseId, signType)` 分支，不新增位置、挑战或协议推断；全量 `ubaa_ui` 测试保持通过，真实账号签到/签退仍未执行。

## 2026-09-02 取消入口状态与截止时间门禁

- 依据冻结 `CgyyOrderDto.canCancelAt`、`displayStatus` 与 `LibBookBookingDto.cancelBlockedMessage`，详情页取消按钮现在同时检查
  场馆订单/审核状态、预约开始前四小时截止、图书馆状态码 `6/8` 及结束/取消状态名；缺少或无法解析场馆状态时不展示入口。
- bridge 订单列表补充公开 `审核状态` 及冻结状态说明，图书馆预约补充公开 `状态码`；不投影交易号、令牌或其它内部字段。新增 widget 回归覆盖已取消、审批驳回、待审核、已过截止和图书馆结束/取消记录，状态门禁不触发准备回调。
- 先加入场馆状态/截止失败回归并观察旧实现展示四个取消按钮，随后完成最小过滤；聚焦与全量 Flutter 测试通过。本轮未访问上游、未执行真实取消或其它账号写入，P3/P4/P5/P6 其余门禁仍未闭合。

## 2026-09-02 `f711f02` 无签名门禁复核

- 取消入口状态修复提交 `f711f02` 已推送；随后 `just refs`、`just check-sensitive`、`just check`、FRB 零漂移、`just flutter-check` 和无签名 RC 前置报告全部通过。
- macOS Debug 宿主构建与产物结构摘要通过（产物大小/摘要仅作安全元数据）；OHOS API26 无签名 Debug HAP 复核通过，包内含 `libs/arm64-v8a/libubaa_bindings.so`。HAP 未签名、未安装、未上传，生成输出已清理。
- 本轮继续不执行真实账号取消或其它写入；原生安全存储 handler、实体设备权限/生命周期、签名发布和 P3/P4/P5/P6 其余完整门禁仍未完成。

## 2026-09-02 场馆审核状态格式默认拒绝
