...
新增公开typed课程/已选/总次数/分类/资料投影，五项RED后六项聚焦GREEN。课程列表整卡进入原有网络详情；已选课程进入UI私有详情页，保留旧“我的课程→课程详情→准备确认→取消回到详情→顶部返回列表”的顺序。私有详情使用同一已选查询中recordId/courseId唯一匹配的当前公开记录，既不新增FeatureQueryView/Bridge合同，也不以课程详情较窄DTO替换并丢失既有签到动作。刷新结果同步同查询的父列表，缺失/重复记录不保留旧动作；不是写入回读证明，最终prepare/commit仍走原协调器和Core。

旧默认四状态与满额显示判断已在UI恢复，只过滤当前服务器页。全部六状态、状态不限、恢复默认在顶栏按需面板，关闭保留，不改变App原有all=true请求；服务端总数保留。完整类别/适用校区字段缺口仍按上文记账，未从名称推断。课程基本信息/时间卡、已选考勤/考核/分数、总体净有效次数和分类三列表已实现，统计null/0和qualified不重算。当前为实施/原生准备中，未宣告本批完成。

原生r1逐图发现长教师/地点撑高列表、56宽详情标签在1.3字体拆字，r2沿旧InfoRow仅列表教师/地点单行省略，详情标签自然宽度且值完整换行。列表选课时间逐条件采用旧`resolveSelectTimeDisplay`：未开始显示开始，否则有截止显示截止，仅开始时显示开始；新增未来/已开始但状态相反的RED后GREEN，证明文案不用status代替时间，不影响Core权威。

读取投影、私有详情和连续滚动已完成588项Flutter门禁及三端38场景；手机平板各96图、macOS7图独立窗口记在old-d3-native。生产签到prepare源码`write/prepare.rs:141–159`本来在target_summary中带签到/签退与时间窗，本批BoyaBackend简写摘要不代表生产缺失，P5需补合成摘要和完整闭环；本批只有prepare/cancel，不记commit成功。
