...
| Judge list/single/batch details | WebVPN auto override | Failed: `upstream_unavailable` (exit 5) | Success: exit 0, `result_count=17` on 2026-08-19; other attempts timed out | Repeated success: latest formal run exit 0, `result_count=17`; earlier successful runs returned `65` standalone and `161` in aggregate. All completed one detail over WebVPN | WebVPN and auto are live-verified. Direct remains unavailable; counts vary with the live assignment set, and intermittent timeout/`upstream_changed` attempts are not treated as success. |

All six rows currently disallow ready-route and network-error fallback. Judge uses a matrix route override, not fallback: the operation starts on WebVPN under `auto`. This is deterministic and intentional until frozen/live evidence proves an idempotent replay contract. The verifier never writes raw bodies, Cookies, tokens, captcha images, complete names, IDs, phones, email addresses, or credentials.
