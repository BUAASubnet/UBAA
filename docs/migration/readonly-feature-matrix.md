...
| Judge list/single/batch details | WebVPN auto override | Failed: `upstream_unavailable` (exit 5) | Success: exit 0, `result_count=17` on 2026-08-19; other attempts timed out | Latest auto attempt timed out at `judge` (exit 5); route diagnostic resolved to WebVPN | WebVPN is the live-verified route. Rerun auto after Judge upstream stabilizes; counts vary with the live assignment set and a timeout is not treated as success. |
