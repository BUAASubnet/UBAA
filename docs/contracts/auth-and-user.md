...
Stable DTOs and error codes are defined by `goal.md` section 6. Legacy authentication envelopes retain schema version 1 for compatibility; read-only and aggregate authentication envelopes use schema version 2 as defined by `docs/contracts/cli-json.schema.json`. Passwords never enter persisted sessions or normal output.
