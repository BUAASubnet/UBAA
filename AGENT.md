...
**问题描述**
- 原有侧边栏使用Row布局，显示时会挤压右侧主内容区域
- 用户体验不佳，内容重排影响阅读连续性

**解决方案**
- 采用Box + overlay模式替代Row布局
- 侧边栏作为悬浮层显示在主内容之上
- 添加半透明背景遮罩，点击可关闭
- 使用Card包装侧边栏，增加Material Design阴影效果
...
Box(modifier = modifier.fillMaxSize()) {
    ...
    if (showSidebar) {
        Box(/* 半透明背景 */)
        Sidebar(modifier = Modifier.align(Alignment.CenterStart))
    }
}
```
...
**解决方案**
- 为MY和ABOUT页面添加独立的TopAppBar
- 实现智能返回逻辑：`navigateBack()` 函数
- 从这两个页面返回时自动显示侧边栏
- 提供一致的返回按钮体验

**导航状态机**
```
HOME -> Sidebar -> MY/ABOUT -> [返回] -> HOME + Sidebar显示
SCHEDULE -> [返回] -> REGULAR
COURSE_DETAIL -> [返回] -> SCHEDULE
```
