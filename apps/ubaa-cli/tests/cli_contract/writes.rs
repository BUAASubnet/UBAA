...
#[tokio::test]
async fn 课堂签到今日与写结果都符合_schema_v5() {
    ...
    assert_eq!(today["schemaVersion"], 5);
...
#[tokio::test]
async fn 图书馆座位原始状态资格与稳定目标符合_schema_v5() {
    let cli = Cli::try_parse_from([
        "ubaa",
        "--json",
        "libbook",
...
#[tokio::test]
async fn 图书馆预约确定成功与业务_false_保持内外层语义并符合_schema_v5() {
...
#[tokio::test]
async fn 博雅三类写操作确认后输出均符合_schema_v5() {
    let cases = [
        (
            vec![
                "ubaa",
