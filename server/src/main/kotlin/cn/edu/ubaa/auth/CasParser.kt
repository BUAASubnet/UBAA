...
/**
 * CAS/SSO 页面解析器。
 * 专门负责解析统一身份认证相关的 HTML 内容，提取登录凭证、验证码标识，并根据页面结构构建 POST 表单参数。
 */
object CasParser {
    private val log = LoggerFactory.getLogger(CasParser::class.java)

    /**
     * 从登录页 HTML 中提取 execution 令牌。
     * 该令牌是 CAS 流程中提交表单所必需的。
     */
    ...
    /**
     * 检测登录页是否包含验证码配置信息。
     * 解析页面中的 JavaScript 配置项 config.captcha。
     *
     * @param html 页面 HTML。
