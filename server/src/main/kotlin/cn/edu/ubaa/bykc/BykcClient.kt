...
/**
 * 博雅系统原始 API 客户端。
 * 负责与博雅系统进行协议层通信，包括鉴权令牌提取、请求加密 (RSA/AES)、响应解密以及原始数据解析。
 *
...
class BykcClient(private val username: String) {
    ...
    private suspend fun ensureSession(): SessionManager.UserSession {
        ...
        suspend fun login(forceRefresh: Boolean = false): Boolean {
            ...
            val token = if (finalUrl.contains("?token=")) {

                finalUrl.substringAfter("?token=")

            } else {

                resp.headers["Location"]?.let { if (it.contains("?token=")) it.substringAfter("?token=") else null }
        ...
        private suspend fun doCallApiRaw(apiName: String, requestJson: String): String {
            ...
            val httpResponse: HttpResponse = client.post(VpnCipher.toVpnUrl("https://bykc.buaa.edu.cn/sscv/$apiName")) {
                ...
                header("ak", enc.ak); header("sk", enc.sk); header("ts", enc.ts)
            ...
            val respBodyText = try { httpResponse.bodyAsText() } catch (_: Exception) { "" }
            ...
            val respBase64 = try { json.decodeFromString<String>(respBodyText) } catch (_: Exception) { respBodyText }
        ...
        suspend fun getUserProfile(): BykcUserProfile {
            ...
            if (!apiResp.isSuccess || apiResp.data == null) throw RuntimeException("BYKC getUserProfile failed: ${apiResp.errmsg}")
        ...
        suspend fun queryStudentSemesterCourseByPage(pageNumber: Int, pageSize: Int): BykcCoursePageResult {
            ...
            if (!apiResp.isSuccess || apiResp.data == null) throw RuntimeException("BYKC query courses failed: ${apiResp.errmsg}")
        ...
        suspend fun getAllConfig(): BykcAllConfig {
            ...
            if (!apiResp.isSuccess || apiResp.data == null) throw RuntimeException("BYKC getAllConfig failed: ${apiResp.errmsg}")
        ...
        suspend fun queryChosenCourse(startDate: String, endDate: String): List<BykcChosenCourse> {
            ...
            if (!apiResp.isSuccess || apiResp.data == null) throw RuntimeException("BYKC queryChosenCourse failed: ${apiResp.errmsg}")
        ...
        suspend fun queryCourseById(id: Long): BykcCourse {
            ...
            if (!apiResp.isSuccess || apiResp.data == null) throw RuntimeException("BYKC queryCourseById failed: ${apiResp.errmsg}")
        ...
        suspend fun signCourse(courseId: Long, lat: Double, lng: Double, signType: Int): BykcApiResponse<BykcSignResult> {
        ...
        suspend fun queryStatisticByUserId(): BykcStatisticsData {
            ...
            if (!apiResp.isSuccess || apiResp.data == null) throw RuntimeException("BYKC queryStatisticByUserId failed: ${apiResp.errmsg}")
