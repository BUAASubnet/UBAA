...
/** VPN URL 转换工具 用于将普通URL转换为北航WebVPN格式的URL，反之亦然 基于原项目的VpnCipher实现 */
object VpnCipher {
    ...
    fun autoDetectEnvironment() {
        ...
        log.info("Auto-detecting network environment...")
        try {
            val url = URL("https://byxt.buaa.edu.cn")
            ...
            if (connection.responseCode == 200 || connection.responseCode == 302) {
                isEnabled = false
                log.info("Direct connection to Intranet successful. VPN mode DISABLED.")
            } else {
                isEnabled = true
                log.info(
                        "Intranet reachable but returned code ${connection.responseCode}. Keeping VPN mode ENABLED."
                )
            }
            ...
            isEnabled = true
            log.info(
                    "Direct connection to Intranet failed (${e.javaClass.simpleName}). VPN mode ENABLED."
            )
        }
    }
    ...
    private fun hexToBytes(hex: String): ByteArray {
        ...
        for (i in 0 until len step 2) {
            result[i / 2] = ((hex[i].digitToInt(16) shl 4) or hex[i + 1].digitToInt(16)).toByte()
        }
        return result
    }
    ...
    fun isVpnUrl(url: String): Boolean {
        return try {
            ...
            // 检查路径格式是否符合VPN URL模式
            parsedUrl.path.isNotEmpty() && parsedUrl.path.substringAfter("/").contains("/")
        } catch (e: Exception) {
