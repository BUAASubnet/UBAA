...
object SigninService {
    ...
    /** 获取或创建指定学生的签到客户端。 */
    private fun getClient(studentId: String): SigninClient = clientCache.getOrPut(studentId) { SigninClient(studentId) }
    ...
    suspend fun performSignin(studentId: String, courseId: String): SigninActionResponse {
        ...
        return SigninActionResponse(code = if (success) 200 else 400, success = success, message = message)
    }
}