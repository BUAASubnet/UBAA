...
fun Route.scheduleRouting() {
        ...
        route("/api/v1/schedule") {
                ...
                get("/weeks") {
                        val username = call.jwtUsername!!
                        val termCode = call.request.queryParameters["termCode"] ?: return@get call.respond(HttpStatusCode.BadRequest)
                        try {
                                val weeks = scheduleService.fetchWeeks(username, termCode)
                                call.respond(HttpStatusCode.OK, weeks)
                        } catch (e: Exception) {
                                call.respond(HttpStatusCode.BadGateway, ErrorResponse(ErrorDetails("error", e.message ?: "Error")))
                        }
                }
                ...
                get("/week") {
                        val username = call.jwtUsername!!
                        val termCode = call.request.queryParameters["termCode"] ?: return@get call.respond(HttpStatusCode.BadRequest)
                        val week = call.request.queryParameters["week"]?.toIntOrNull() ?: return@get call.respond(HttpStatusCode.BadRequest)
                        try {
