...
fun Route.scheduleRouting() {
        ...
        route("/api/v1/schedule") {
                get("/terms") {
                        ...
                        application.log.info("Fetching terms for user: {}", username)

                        try {
                                val terms = scheduleService.fetchTerms(username)
                                application.log.info(
                                        "Terms fetched successfully for user: {}",
                                        username
                                )
                                call.respond(HttpStatusCode.OK, terms)
                        } catch (e: LoginException) {
                                application.log.warn(
                                        "Terms fetch failed for user {}: {}",
                                        username,
                                        e.message
                                )
                                call.respond(
                                ...
                                call.application.environment.log.error(
                                        "Unexpected error while fetching terms.",
                                        e
                                )
                                call.respond(
                ...
                get("/weeks") {
                        ...
                        if (termCode.isNullOrBlank()) {
                                call.respond(
                                        ...
                                        ErrorResponse(
                                                ErrorDetails(
                                                        "invalid_request",
                                                        "termCode parameter is required"
                                                )
                                        )
                                )
                                return@get
                        ...
                        try {
                                ...
                                call.application.environment.log.error(
                                        "Unexpected error while fetching weeks.",
                                        e
                                )
                                call.respond(
                                        HttpStatusCode.InternalServerError,
                ...
                get("/week") {
                        ...
                        try {
                                val schedule =
                                        scheduleService.fetchWeeklySchedule(
                                                username,
                                                termCode,
                                                week
                                        )
