...
class ClassroomViewModel(private val api: ClassroomApi = ClassroomApi()) : ViewModel() {
    ...
    /** 
     * 根据搜索关键字过滤后的教室数据流。 
     * 自动响应 uiState 和 searchQuery 的变化。
     */
    val filteredData: StateFlow<Map<String, List<ClassroomInfo>>> =
            combine(_uiState, _searchQuery) { state, query ->
                        if (state is ClassroomUiState.Success) {
                            ...
                            else allData.mapValues { (_, list) -> list.filter { it.name.contains(query, true) } }
    ...
    /** 切换校区并自动重新查询。 */
    fun setXqid(id: Int) { _xqid.value = id; query() }

    /** 切换日期并自动重新查询。 */
    fun setDate(date: String) { _date.value = date; query() }
